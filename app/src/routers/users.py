...
async def get_user(
    ...
    user = database_actions.get_user_by_email(
        database, str(current_user.email)
    )
    if user is None:
        raise HTTPException(
    ...
    return JSONResponse(
        status_code=status.HTTP_200_OK,
        content={'success': jsonable_encoder(show_user)},
    )


@router.put('/me')
async def update_user(
    new_user_information: UserBase,
...
async def update_user_information(
    new_user_information: UserOptional,
    database_session: Session = Depends(database_actions.get_database),
    current_user: User_Model.User = Depends(get_current_user),
):
    stored_user = database_actions.get_user_by_email(
...
async def delete_user(
    ...
    return JSONResponse(
        status_code=status.HTTP_200_OK,
        content=database_actions.delete_user(database_session, stored_user),
    )
